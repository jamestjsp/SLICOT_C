/**
 * @file ab05od.h
 * @brief C wrapper for SLICOT routine AB05OD
 *
 * This file provides a C interface to the SLICOT routine AB05OD,
 ...
 */
 ...
 /**
  * @brief Computes the rowwise concatenation of two state-space systems.
  *
  * Calculates the state-space representation (A,B,C,D) of the system
  * formed by the parallel connection Y = Y1 + alpha*Y2, where
  * System 1 is (A1,B1,C1,D1) with input U, and
  * System 2 is (A2,B2,C2,D2) with input V.
  * The resulting system has input [U; V].
  *
  ...
  * @param[in] p1        Number of outputs for both systems (rows of C1, D1, C2, D2), p1 >= 0.
  * @param[in] n2        Order of system 2 (matrix A2), n2 >= 0.
  * @param[in] m2        Number of inputs for system 2 (columns of B2, D2), m2 >= 0.
  * @param[in] alpha     Coefficient multiplying the output equation of system 2.
  ...
  * @param[in] c2        Double array for matrix C2 (System 2). Dimension (ldc2, n2) or (p1, ldc2).
  * @param[in] ldc2      Leading dimension of C2. >= max(1,p1) if n2>0, else >=1.
  * @param[in] d2        Double array for matrix D2 (System 2). Dimension (ldd2, m2) or (p1, ldd2).
  * @param[in] ldd2      Leading dimension of D2. >= max(1,p1).
  ...
  * @param[out] d        Double array for the resulting matrix D. Dimension (ldd, m) or (p1, ldd).
