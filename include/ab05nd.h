/**
 * @file ab05nd.h
 * @brief C wrapper for SLICOT routine AB05ND
 *
 * This file provides a C interface to the SLICOT routine AB05ND,
 ...
 */
 ...
 /**
  * @brief Computes the feedback connection of two state-space systems.
  *
  * Calculates the state-space representation (A,B,C,D) of the system
  * formed by the feedback connection of System 1 (A1,B1,C1,D1) and
  * System 2 (A2,B2,C2,D2), according to the equations:
  * U1 = U + alpha*Y2
  * Y  = Y1 = U2
  *
  ...
  * @param[in] m1        Number of inputs for system 1 / outputs for system 2, m1 >= 0.
  * @param[in] p1        Number of outputs for system 1 / inputs for system 2, p1 >= 0.
  * @param[in] n2        Order of system 2 (matrix A2), n2 >= 0.
  * @param[in] alpha     Feedback gain coefficient (e.g., +1 for positive, -1 for negative feedback).
  ...
  * @param[in] b2        Double array for matrix B2 (System 2). Dimension (ldb2, p1) or (n2, ldb2).
  * @param[in] ldb2      Leading dimension of B2. >= max(1,n2).
  * @param[in] c2        Double array for matrix C2 (System 2). Dimension (ldc2, n2) or (m1, ldc2).
  * @param[in] ldc2      Leading dimension of C2. >= max(1,m1) if n2>0, else >=1.
  * @param[in] d2        Double array for matrix D2 (System 2). Dimension (ldd2, p1) or (m1, ldd2).
  * @param[in] ldd2      Leading dimension of D2. >= max(1,m1).
  ...
  * @param[out] b        Double array for the resulting matrix B. Dimension (ldb, m1) or (n, ldb).
  * @param[in] ldb       Leading dimension of B. >= max(1, n1+n2).
  * @param[out] c        Double array for the resulting matrix C. Dimension (ldc, n) or (p1, ldc).
  * @param[in] ldc       Leading dimension of C. >= max(1,p1) if n1+n2>0, else >=1.
  * @param[out] d        Double array for the resulting matrix D. Dimension (ldd, m1) or (p1, ldd).
  * @param[in] ldd       Leading dimension of D. >= max(1,p1).
