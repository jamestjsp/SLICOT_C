/**
 * @file ab05pd.h
 * @brief C wrapper for SLICOT routine AB05PD
 *
 * This file provides a C interface to the SLICOT routine AB05PD,
 ...
 */
 ...
 /**
  * @brief Computes the parallel connection (sum) of two state-space systems.
  *
  * Calculates the state-space representation (A,B,C,D) of the system
  * G = G1 + alpha*G2, where G1 = (A1,B1,C1,D1) and G2 = (A2,B2,C2,D2).
  * Both systems must have the same number of inputs (M) and outputs (P).
  *
  ...
  * @param[in] m         Number of inputs for both systems (columns of B1, D1, B2, D2), m >= 0.
  * @param[in] p         Number of outputs for both systems (rows of C1, D1, C2, D2), p >= 0.
  * @param[in] n2        Order of system 2 (matrix A2), n2 >= 0.
  * @param[in] alpha     Coefficient multiplying system G2.
  ...
  * @param[in] b1        Double array for matrix B1 (System 1). Dimension (ldb1, m) or (n1, ldb1).
  * @param[in] ldb1      Leading dimension of B1. >= max(1,n1).
  * @param[in] c1        Double array for matrix C1 (System 1). Dimension (ldc1, n1) or (p, ldc1).
  * @param[in] ldc1      Leading dimension of C1. >= max(1,p) if n1>0, else >=1.
  * @param[in] d1        Double array for matrix D1 (System 1). Dimension (ldd1, m) or (p, ldd1).
  * @param[in] ldd1      Leading dimension of D1. >= max(1,p).
  ...
  * @param[in] b2        Double array for matrix B2 (System 2). Dimension (ldb2, m) or (n2, ldb2).
  * @param[in] ldb2      Leading dimension of B2. >= max(1,n2).
  * @param[in] c2        Double array for matrix C2 (System 2). Dimension (ldc2, n2) or (p, ldc2).
  * @param[in] ldc2      Leading dimension of C2. >= max(1,p) if n2>0, else >=1.
  * @param[in] d2        Double array for matrix D2 (System 2). Dimension (ldd2, m) or (p, ldd2).
  * @param[in] ldd2      Leading dimension of D2. >= max(1,p).
  ...
  * @param[in] ldc       Leading dimension of C. >= max(1,p) if n1+n2>0, else >=1.
 ...
 int slicot_ab05pd(char over,
                   int n1, int m, int p, int n2, double alpha,
