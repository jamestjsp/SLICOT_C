/**
 * @file ab05md.h
 * @brief C wrapper for SLICOT routine AB05MD
 *
 * This file provides a C interface to the SLICOT routine AB05MD,
 * which computes the state-space model (A,B,C,D) for the cascaded
 * inter-connection of two systems, (A1,B1,C1,D1) -> (A2,B2,C2,D2).
 */
 ...
 /**
  * @brief Computes the cascade connection of two state-space systems.
  *
  * Calculates the state-space representation (A,B,C,D) of the system
  * formed by cascading System 1 (A1,B1,C1,D1) followed by System 2
  * (A2,B2,C2,D2), where the output of System 1 is the input to System 2.
  *
  * Note: This wrapper assumes OVER = 'N' (no overlapping arrays).
  * The DWORK workspace is not used.
  *
  * @param[in] uplo      Specifies the desired structure for the resulting matrix A:
  * = 'U': Upper block triangular form.
  * = 'L': Lower block triangular form.
  * @param[in] over      Specifies array overlapping (Not supported by wrapper, assumed 'N'):
  * = 'N': Do not overlap arrays.
  * = 'O': Overlap pairs of arrays (requires DWORK, not handled here).
  * @param[in] n1        Order of system 1 (matrix A1), n1 >= 0.
  * @param[in] m1        Number of inputs for system 1 (columns of B1, D1), m1 >= 0.
  * @param[in] p1        Number of outputs for system 1 / inputs for system 2 (rows of C1, D1; columns of B2, D2), p1 >= 0.
  ...
  * @param[in] d2        Double array for matrix D2 (System 2). Dimension (ldd2, p1) or (p2, ldd2).
  ...
  * @param[out] c        Double array for the resulting matrix C. Dimension (ldc, n) or (p2, ldc).
  * @param[in] ldc       Leading dimension of C. >= max(1,p2) if n1+n2>0, else >=1.
  * @param[out] d        Double array for the resulting matrix D. Dimension (ldd, m1) or (p2, ldd).
  * @param[in] ldd       Leading dimension of D. >= max(1,p2).
