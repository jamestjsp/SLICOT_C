 ...
 */
  ...
  * (column-major) matrix, performing the necessary index transformation and respecting
                                          ...
                                          int ld_src, int ld_dest, size_t elem_size)
 {
     ...
     if (!src || !dest || rows <= 0 || cols <= 0 || ld_src < cols || ld_dest < rows || elem_size <= 0)
     {
         // ld_src (C cols) must be >= cols to copy
         // ld_dest (Fortran rows) must be >= rows to copy
         return; // Or handle error appropriately
     }
     ...
     { // Iterate through rows
         for (j = 0; j < cols; j++)
         { // Iterate through columns
             // Source index (row-major): element at [i][j] is at offset (i * ld_src + j)
             size_t src_offset = ((size_t)i * ld_src + j) * elem_size;
             // Destination index (column-major): element at [i][j] is at offset (i + j * ld_dest)
             size_t dest_offset = (i + (size_t)j * ld_dest) * elem_size;
                                    ...
                                    int ld_src, int ld_dest, size_t elem_size)
 {
     ...
     {
         // ld_src (Fortran rows) must be >= rows to copy
         // ld_dest (C cols) must be >= cols to copy
         return; // Or handle error appropriately
     }
 ...
 void set_identity(int n, double *mat, int ld, int row_major)
 {
     if (!mat)
         return;
     // Calculate total size carefully to avoid overflow and access violation
     size_t num_elements;
     if (row_major) {
         // Row-major: ld is number of columns, need n rows
         num_elements = (size_t)n * ld;
     } else {
         // Column-major: ld is number of rows, need n columns
         num_elements = (size_t)ld * n;
     }
     memset(mat, 0, num_elements * sizeof(double)); // Zero out the matrix first
 
     if (row_major)
     {
         // mat[i * ld + i] = 1.0; ld is cols
         for (int i = 0; i < n; ++i)
         {
            // Check row index i < n (implicit) and column index i < ld
            if (i < ld) {
     ...
     {
         // mat[i + i * ld] = 1.0; ld is rows
         for (int i = 0; i < n; ++i)
         {
             // Check row index i < ld and column index i < n (implicit)
             if (i < ld) {
  ...
  * the other triangle by symmetry. Assumes source has C leading dimension = n.
  *
 ...
 void slicot_transpose_symmetric_to_fortran(const void *src, void *dest, int n, char uplo, size_t elem_size)
 {
     ...
     int i, j; // Row and column indices
     char uplo_upper = toupper(uplo);
     int ld_src = n; // Assumed C leading dimension (cols) for row-major source
     int ld_dest = n; // Assumed Fortran leading dimension (rows) for column-major dest
     ...
     if (uplo_upper == 'U') {
         ...
         for (i = 0; i < n; i++) { // Iterate through rows
             for (j = i; j < n; j++) { // Iterate through columns (from diagonal right)
                 ...
                 // Destination index (column-major): element at [i][j] is at offset (i + j * ld_dest)
                 size_t dest_offset_ij = (i + (size_t)j * ld_dest) * elem_size;
                 // Destination index for symmetric part [j][i]: offset (j + i * ld_dest)
                 size_t dest_offset_ji = (j + (size_t)i * ld_dest) * elem_size;
  ...
  * Assumes destination has C leading dimension = n.
  *
 ...
 void slicot_transpose_symmetric_to_c(const void *src, void *dest, int n, char uplo, size_t elem_size)
 {
     ...
     int i, j; // Row and column indices
     char uplo_upper = toupper(uplo);
     int ld_src = n; // Assumed Fortran leading dimension (rows) for column-major source
     int ld_dest = n; // Assumed C leading dimension (cols) for row-major destination
     ...
     if (uplo_upper == 'U') {
         ...
         for (i = 0; i < n; i++) { // Row index
             for (j = i; j < n; j++) { // Column index (from diagonal right)
                 // Source index (col-major): element at [i][j] is at offset (i + j * ld_src)
                 size_t src_offset = (i + (size_t)j * ld_src) * elem_size;
                 // Destination index (row-major): element at [i][j] is at offset (i * ld_dest + j)
                 size_t dest_offset = ((size_t)i * ld_dest + j) * elem_size;
                 // Copy the element
 ...
 void printMatrixD(const char* name, const double* data, int rows, int cols, int ld, int rowMajor) {
     ...
     for (int i = 0; i < rows; ++i) {
         ...
         for (int j = 0; j < cols; ++j) {
             double val;
             if (rowMajor) {
                  // Check bounds: i < rows (implicit), j < ld (cols)
                 if (j >= ld) { printf(" OOB "); continue; } // Out of bounds access
                 val = data[(size_t)i * ld + j];
             } else {
                  // Check bounds: i < ld (rows), j < cols (implicit)
                 if (i >= ld) { printf(" OOB "); continue; } // Out of bounds access
                 val = data[i + (size_t)j * ld];
             }
