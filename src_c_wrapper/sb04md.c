/**
 * @file sb04md.c
 * @brief C wrapper implementation for SLICOT routine SB04MD
 *
 * This file provides a C wrapper implementation for the SLICOT routine SB04MD,
 * which solves the continuous-time Sylvester equation AX + XB = C
 ...
 */
  ...
  */
 extern void F77_FUNC(sb04md, SB04MD)(
                   ...
                   int row_major)
 {
     ...
     ldwork = -1; // Query mode
     F77_FUNC(sb04md, SB04MD)(&n, &m, a_ptr, &lda_f, b_ptr, &ldb_f, c_ptr, &ldc_f, z_ptr, &ldz_f,
     ...
     // Check against minimum documented size: MAX(1, 2*N*N + 8*N, 5*M, N + M)
     int min_ldwork = 1;
     min_ldwork = MAX(min_ldwork, 2 * n * n + 8 * n);
