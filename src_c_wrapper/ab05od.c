 ...
 */
 ...
 int slicot_ab05od(char over,
                   ...
                   int* n_out, int* m_out, // Renamed to avoid conflict
                   ...
                   int row_major)
 {
     ...
     int min_ldc2_f = MAX(1, p1); int min_ldd2_f = MAX(1, p1); // Rows=P1 (C2 is P1-by-N2, D2 is P1-by-M2)
     // Check leading dimensions for outputs
     int min_lda_f = MAX(1, n); int min_ldb_f = MAX(1, n);
     int min_ldc_f = MAX(1, p1); int min_ldd_f = MAX(1, p1); // Rows=P1 (C is P1-by-N, D is P1-by-M)

     if (row_major) {
         // Check C dimensions (number of columns)
         if (n1 > 0 && lda1 < n1) { info = -9; goto cleanup; }
         if (n1 > 0 && ldb1 < m1) { info = -11; goto cleanup; }
         ...
         if (p1 > 0 && ldc2 < n2) { info = -21; goto cleanup; } // C2 is P1-by-N2
         if (p1 > 0 && ldd2 < m2) { info = -23; goto cleanup; } // D2 is P1-by-M2
         // Check output C dimensions
         if (n > 0 && lda < n) { info = -26; goto cleanup; }
         if (n > 0 && ldb < m) { info = -28; goto cleanup; } // B is N-by-M
         if (p1 > 0 && ldc < n) { info = -30; goto cleanup; } // C is P1-by-N
         if (p1 > 0 && ldd < m) { info = -32; goto cleanup; } // D is P1-by-M
     } else {
     ...
     if (row_major) {
         ...
         size_t c2_size = (size_t)p1 * n2; if (p1 == 0 || n2 == 0) c2_size = 0; // P1 rows, N2 cols
         size_t d2_size = (size_t)p1 * m2; if (p1 == 0 || m2 == 0) d2_size = 0; // P1 rows, M2 cols
         ...
         size_t d_size = (size_t)p1 * m; if (p1 == 0 || m == 0) d_size = 0; // P1 rows, M cols
         ...
         if (c2_size > 0) slicot_transpose_to_fortran(c2, c2_cm, p1, n2, sizeof(double)); // P1 rows, N2 cols
         if (d2_size > 0) slicot_transpose_to_fortran(d2, d2_cm, p1, m2, sizeof(double)); // P1 rows, M2 cols
         ...
         ldc2_f = MAX(1, p1); ldd2_f = MAX(1, p1);
     ...
     if (row_major && info == 0) {
         ...
         if (d_size > 0) slicot_transpose_to_c(d_cm, d, p1, m, sizeof(double)); // P1 rows, M cols
     }
