    ...
    int row_major)
{
    ...
    int *iwork = NULL; double *dwork = NULL; int *bwork_c = NULL;
    int liwork = 0, ldwork = 0, lbwork = 0;
    ...
    char dico_f = toupper(dico_param); char jobb_f = toupper(jobb_param);
    char fact_f = toupper(fact_param); char uplo_f = toupper(uplo_param);
    char jobl_f = toupper(jobl_param); char scal_f = toupper(scal_param);
    char sort_f = toupper(sort_param); char acc_f  = toupper(acc_param);
    ...
    if (q_io == NULL && ( (n_param > 0 && (fact_f == 'N' || fact_f == 'D')) || (p_param > 0 && n_param > 0 && (fact_f == 'C' || fact_f == 'B')) ) ) { info = -18; goto cleanup; }
    if (jobb_f == 'B' && r_io == NULL && ( (m_param > 0 && (fact_f == 'N' || fact_f == 'C')) || (p_param > 0 && m_param > 0 && (fact_f == 'D' || fact_f == 'B')) ) ) { info = -20; goto cleanup; }
    if (jobb_f == 'B' && jobl_f == 'N' && l_io == NULL && n_param > 0 && m_param > 0) { info = -22; goto cleanup; }
    
    if (rcondu == NULL) { info = -23; goto cleanup; } // Argument index based on typical Fortran order
    if (x_out == NULL && n_param > 0) { info = -24; goto cleanup; }
    if (alfar_out == NULL && n_param > 0) { info = -26; goto cleanup; }
    ...
    if (iwarn_out == NULL) { info = -39; goto cleanup; } // Adjusted index for IWARN

    int min_lda_f = MAX(1, n_param); int min_lde_f = MAX(1, n_param);
    ...
    int lda_c_cols = n_param; int lde_c_cols = n_param;
    int ldb_c_cols = (jobb_f == 'B') ? m_param : n_param;
    int ldq_c_cols = (fact_f == 'N' || fact_f == 'D') ? n_param : n_param; 
    int ldr_c_cols = 0; if (jobb_f == 'B') ldr_c_cols = (fact_f == 'N' || fact_f == 'C') ? m_param : m_param;
    int ldl_c_cols = 0; if (jobb_f == 'B' && jobl_f == 'N') ldl_c_cols = m_param;
    int ldx_c_cols = n_param;
    int s_actual_cols_f = (jobb_f == 'B') ? (2*n_param + m_param) : (2*n_param);
    int lds_c_cols = s_actual_cols_f;
    ...
    if (row_major) {
        ...
        if ( ( (fact_f == 'N' || fact_f == 'D') && n_param > 0 ) || ( (fact_f == 'C' || fact_f == 'B') && p_param > 0 && n_param > 0 ) ) { if (ldq < ldq_c_cols) {info = -19; goto cleanup;} }
        if (jobb_f == 'B' && ( (m_param > 0 && (fact_f == 'N' || fact_f == 'C')) || (p_param > 0 && m_param > 0 && (fact_f == 'D' || fact_f == 'B')) ) ) { if (ldr < ldr_c_cols) {info = -21; goto cleanup;} }
        ...
        if ( ( (fact_f == 'N' || fact_f == 'D') && n_param > 0 ) || ( (fact_f == 'C' || fact_f == 'B') && p_param > 0 && n_param > 0 ) ) { if (ldq < min_ldq_f) {info = -19; goto cleanup;} }
        if (jobb_f == 'B' && ( (m_param > 0 && (fact_f == 'N' || fact_f == 'C')) || (p_param > 0 && m_param > 0 && (fact_f == 'D' || fact_f == 'B')) ) ) { if (ldr < min_ldr_f) {info = -21; goto cleanup;} }
    ...
    if (jobb_f == 'B') liwork = MAX(1, MAX(m_param, 2 * n_param));
    else liwork = MAX(1, 2 * n_param);
    iwork = (int*)malloc((size_t)liwork * sizeof(int)); CHECK_ALLOC(iwork);

    lbwork = 2 * n_param;
    if (lbwork > 0) {bwork_c = (int*)malloc((size_t)lbwork * sizeof(int)); CHECK_ALLOC(bwork_c);}
    else {bwork_c = NULL;}

    if (jobb_f == 'G') ldwork = MAX(1, MAX(7 * (2 * n_param + 1) + 16, 16 * n_param));
    else ldwork = MAX(1, MAX(7 * (2 * n_param + 1) + 16, MAX(16 * n_param, MAX(2 * n_param + m_param, 3 * m_param))));
    ...
    size_t b_nelems = (jobb_f == 'B') ? (size_t)n_param * m_param : (size_t)n_param * n_param;
    if(n_param == 0 || (jobb_f == 'B' && m_param == 0 && n_param > 0) || (jobb_f == 'G' && n_param == 0) ) b_nelems = 0;
    
    size_t q_nelems = (fact_f == 'N' || fact_f == 'D') ? (size_t)n_param*n_param : (size_t)p_param*n_param;
    if( ((fact_f == 'N' || fact_f == 'D') && n_param == 0) || ((fact_f == 'C' || fact_f == 'B') && (p_param == 0 || n_param == 0)) ) q_nelems = 0;
    
    size_t r_nelems = 0; if (jobb_f == 'B') r_nelems = (fact_f == 'N' || fact_f == 'C') ? (size_t)m_param*m_param : (size_t)p_param*m_param;
    if(jobb_f == 'B' && ( ((fact_f == 'N' || fact_f == 'C') && m_param == 0) || ((fact_f == 'D' || fact_f == 'B') && (p_param == 0 || m_param == 0)) ) ) r_nelems = 0;
    
    size_t l_nelems = 0; if (jobb_f == 'B' && jobl_f == 'N') l_nelems = (size_t)n_param * m_param;
    if(jobb_f == 'B' && jobl_f == 'N' && (n_param == 0 || m_param == 0)) l_nelems = 0;

    size_t x_nelems = (size_t)n_param * n_param; if(n_param==0) x_nelems=0;
    size_t s_alloc_nelems=0, t_alloc_nelems=0, u_alloc_nelems=0;
    if (n_param > 0) {
        s_alloc_nelems = (size_t)min_lds_f * s_actual_cols_f;
        t_alloc_nelems = (size_t)min_ldt_f * (2*n_param);
        u_alloc_nelems = (size_t)min_ldu_f * (2*n_param);
    }
    ...
    if (row_major) {
        ...
        if (r_nelems > 0 && jobb_f == 'B') { r_cm = (double*)malloc(r_nelems * sizeof(double)); CHECK_ALLOC(r_cm); }
        if (l_nelems > 0 && jobb_f == 'B' && jobl_f == 'N') { l_cm = (double*)malloc(l_nelems * sizeof(double)); CHECK_ALLOC(l_cm); }
        if (x_nelems > 0) { x_cm = (double*)malloc(x_nelems * sizeof(double)); CHECK_ALLOC(x_cm); }
        if (s_alloc_nelems > 0) { s_cm = (double*)malloc(s_alloc_nelems * sizeof(double)); CHECK_ALLOC(s_cm); }
        if (t_alloc_nelems > 0) { t_cm = (double*)malloc(t_alloc_nelems * sizeof(double)); CHECK_ALLOC(t_cm); }
        if (u_alloc_nelems > 0) { u_cm = (double*)malloc(u_alloc_nelems * sizeof(double)); CHECK_ALLOC(u_cm); }
    }
    ...
    if (row_major) {
        lda_f = min_lda_f; lde_f = min_lde_f; ldb_f = min_ldb_f; ldq_f = min_ldq_f; 
        ldr_f = min_ldr_f; ldl_f = min_ldl_f; ldx_f = min_ldx_f; 
        ...
        if (b_nelems>0) {slicot_transpose_to_fortran_with_ld(b_io,b_cm,n_param,(jobb_f=='B'?m_param:n_param),ldb,ldb_f,sizeof(double));b_ptr=b_cm;} else b_ptr=NULL;
        if (q_nelems>0) {slicot_transpose_to_fortran_with_ld(q_io,q_cm,(fact_f=='N'||fact_f=='D'?n_param:p_param),n_param,ldq,ldq_f,sizeof(double));q_ptr=q_cm;} else q_ptr=NULL;
        if (r_nelems>0 && jobb_f=='B') {slicot_transpose_to_fortran_with_ld(r_io,r_cm,(fact_f=='N'||fact_f=='C'?m_param:p_param),m_param,ldr,ldr_f,sizeof(double));r_ptr=r_cm;} else if (jobb_f=='B') r_ptr=NULL;
        if (l_nelems>0 && jobb_f=='B' && jobl_f=='N') {slicot_transpose_to_fortran_with_ld(l_io,l_cm,n_param,m_param,ldl,ldl_f,sizeof(double));l_ptr=l_cm;} else if (jobb_f=='B' && jobl_f=='N') l_ptr=NULL;
        
        if (x_nelems>0) x_ptr=x_cm; else x_ptr=NULL;
        if (s_alloc_nelems>0) s_ptr=s_cm; else s_ptr=NULL;
        if (t_alloc_nelems>0) t_ptr=t_cm; else t_ptr=NULL;
        if (u_alloc_nelems>0) u_ptr=u_cm; else u_ptr=NULL;
    } else {
        if(a_nelems==0)a_ptr=NULL; if(e_nelems==0)e_ptr=NULL; if(b_nelems==0)b_ptr=NULL;
        if(q_nelems==0)q_ptr=NULL; if(jobb_f=='B'&&r_nelems==0)r_ptr=NULL; 
        if(jobb_f=='B'&&jobl_f=='N'&&l_nelems==0)l_ptr=NULL;
        if(x_nelems==0)x_ptr=NULL; if(s_alloc_nelems==0)s_ptr=NULL; 
        if(t_alloc_nelems==0)t_ptr=NULL; if(u_alloc_nelems==0)u_ptr=NULL;
    }
    ...
    F77_FUNC(sg02ad,SG02AD)(&dico_f,&jobb_f,&fact_f,&uplo_f,&jobl_f,&scal_f,&sort_f,&acc_f,
                             ...
                             &local_iwarn,&info,1,1,1,1,1,1,1,1);

    if(iwarn_out!=NULL) *iwarn_out = local_iwarn;

    if (row_major && info == 0) {
        if (x_nelems > 0 && x_cm != NULL && x_out != NULL) slicot_transpose_to_c_with_ld(x_cm,x_out,n_param,n_param,ldx_f,ldx,sizeof(double));
        if (n_param > 0) { // S, T, U only if N > 0
            int s_rows_to_copy = 2*n_param; int s_cols_to_copy = s_actual_cols_f;
            int t_rows_to_copy = 2*n_param; int t_cols_to_copy = 2*n_param;
            int u_rows_to_copy = 2*n_param; int u_cols_to_copy = 2*n_param;
            if (s_alloc_nelems > 0 && s_cm != NULL && s_out != NULL) slicot_transpose_to_c_with_ld(s_cm,s_out,s_rows_to_copy,s_cols_to_copy,lds_f,lds,sizeof(double));
            if (t_alloc_nelems > 0 && t_cm != NULL && t_out != NULL) slicot_transpose_to_c_with_ld(t_cm,t_out,t_rows_to_copy,t_cols_to_copy,ldt_f,ldt,sizeof(double));
            if (u_alloc_nelems > 0 && u_cm != NULL && u_out != NULL) slicot_transpose_to_c_with_ld(u_cm,u_out,u_rows_to_copy,u_cols_to_copy,ldu_f,ldu,sizeof(double));
        }
    }

cleanup:
    free(iwork); free(dwork); free(bwork_c);
    if(row_major){
