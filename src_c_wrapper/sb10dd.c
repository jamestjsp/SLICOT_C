 ...
 */
                   ...
                   int row_major)
 {
     ...
     F77_FUNC(sb10dd, SB10DD)(&n, &m, &np, &ncon, &nmeas, &gamma,
                              ...
                              x, &ldx, z, &ldz, rcond, &tol,
                              iwork, &dwork_query, &ldwork, bwork, &info);
 
 
     if (info < 0 && info != -30) { info = info; goto cleanup; } // Query failed due to invalid argument
     ...
     if (row_major) {
         ...
         if (info == 0) {
             ...
             if (x_size > 0) slicot_transpose_symmetric_to_c(x_cm, x, n, 'U', elem_size); // X is symmetric
             if (z_size > 0) slicot_transpose_symmetric_to_c(z_cm, z, n, 'U', elem_size); // Z is symmetric
