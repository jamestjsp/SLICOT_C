                  ...
                  int row_major)
{
    ...
    // Allocate DWORK - calculate required workspace size
    // For OVER='N': LDWORK >= MAX(1, P1*P1, M1*M1, N1*P1)
    // For OVER='O' (not supported by this wrapper):
    //   - If M1 <= N*N2: LDWORK >= MAX(1, N1*P1 + MAX(P1*P1, M1*M1, N1*P1))
    //   - Otherwise:     LDWORK >= MAX(1, N1*P1 + MAX(P1*P1, M1*(M1+1), N1*P1))
