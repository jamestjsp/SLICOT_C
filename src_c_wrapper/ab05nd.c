 ...
 */
                   ...
                   int row_major)
 {
     ...
     // No check for ALPHA needed based on docs, but could add if desired
     ...
     if (row_major) {
         ...
         size_t c2_size = (size_t)m1 * n2; if (m1 == 0 || n2 == 0) c2_size = 0; // Rows=M1
         size_t d2_size = (size_t)m1 * p1; if (m1 == 0 || p1 == 0) d2_size = 0; // Rows=M1
         ...
         size_t d_size = (size_t)p1 * m1; if (p1 == 0 || m1 == 0) d_size = 0; // Rows=P1
         ...
         if (c2_size > 0) slicot_transpose_to_fortran(c2, c2_cm, m1, n2, sizeof(double)); // Rows=M1
         if (d2_size > 0) slicot_transpose_to_fortran(d2, d2_cm, m1, p1, sizeof(double)); // Rows=M1
         ...
         int ldc2_f = (m1 > 0) ? m1 : 1; int ldd2_f = (m1 > 0) ? m1 : 1; // Rows=M1
         ...
         if (info == 0) {
             ...
             if (d_size > 0) slicot_transpose_to_c(d_cm, d, p1, m1, sizeof(double)); // Rows=P1
         }
