 ...
 */
                   ...
                   double tol, int row_major)
 {
     ...
     int* iwork = NULL;
     int iwork_size = 0;
     int n_val = 0; // Max possible order N = sum(index)
 
     const int rowcol_len = 1;
     char rowcol_upper = toupper(rowcol);
 
     /* Pointers for column-major copies if needed */
     double *dcoeff_cm = NULL, *ucoeff_cm = NULL; // For inputs
     ...
     int min_lduco1_f = (rowcol_upper == 'R') ? MAX(1, p) : MAX(1, maxmp);
     int min_lduco2_f = (rowcol_upper == 'R') ? MAX(1, m) : MAX(1, maxmp);
 
 
     if (row_major) {
         // For row-major C, LD is number of columns
         int min_lda_rm_cols = n_val; // Use max N for allocation check
         int min_ldb_rm_cols = maxmp; // B needs workspace
         int min_ldc_rm_cols = n_val; // Use max N for allocation check
     ...
     // Allocate IWORK
     iwork_size = n_val + maxmp; // Size is N + MAX(M,P)
     ...
     ldwork = -1; // Query mode
     int nr_dummy = 0; // Use dummy output vars for query
     F77_FUNC(td04ad, TD04AD)(&rowcol_upper, &m, &p, index,
                              NULL, &lddcoe, NULL, &lduco1, &lduco2,
                              &nr_dummy, NULL, &lda, NULL, &ldb, NULL, &ldc, NULL, &ldd,
     ...
     int min_ldwork = MAX(1, n_val + MAX(n_val, MAX(3*m, 3*p)));
     ...
     size_t dcoeff_slice_size = (size_t)porm * kdcoef;
     size_t ucoeff_slice_size = (rowcol_upper == 'R') ? (size_t)p * m : (size_t)p * m; // Assuming PxM based on example
     size_t ucoeff_total_size = ucoeff_slice_size * kdcoef;
 
     // Sizes for output state-space matrices (use max N for allocation)
     ...
     size_t d_rows = (rowcol_upper == 'R') ? p : maxmp; size_t d_cols = (rowcol_upper == 'R') ? m : maxmp; size_t d_size = d_rows * d_cols; // Use max for workspace if 'C'
 
 
     // Need mutable copy of ucoeff for Fortran call if ROWCOL='C'
     // Allocate memory for mutable copies / transposed inputs
     if (porm > 0 && kdcoef > 0) { dcoeff_cm = (double*)malloc((size_t)porm * kdcoef * elem_size); CHECK_ALLOC(dcoeff_cm); }
     ...
     if (row_major) {
         ...
         for (int k = 0; k < kdcoef; ++k) {
              size_t offset = k * ucoeff_slice_size;
              if (ucoeff_slice_size > 0) {
                  // Assuming U is PxM for transposition
                  slicot_transpose_to_fortran(ucoeff + offset, ucoeff_cm + offset, p, m, elem_size);
              }
         }
         ...
         int lduco1_f = (p > 0) ? p : 1; // Assuming PxM
         int lduco2_f = (p > 0) ? p : 1; // Assuming PxM? Example uses MAXMP. Let's use P.
         ...
         F77_FUNC(td04ad, TD04AD)(&rowcol_upper, &m, &p, index,
                                  dcoeff_cm, &lddcoe_f, ucoeff_cm, &lduco1_f, &lduco2_f,
                                  nr, a_cm, &lda_f, b_cm, &ldb_f, c_cm, &ldc_f, d_cm, &ldd_f,
         ...
         /* --- Column-Major Case --- */
         /* Copy const inputs to mutable temps if needed (UCOEFF if ROWCOL='C') */
         if (porm > 0 && kdcoef > 0) memcpy(dcoeff_cm, dcoeff, (size_t)porm * kdcoef * elem_size); // Copy dcoeff always? Fortran takes const.. safer to copy.
         if (ucoeff_total_size > 0) memcpy(ucoeff_cm, ucoeff, ucoeff_total_size * elem_size); // Copy ucoeff always? Fortran takes non-const.. must copy.
 
         /* Call the Fortran routine directly with user-provided arrays */
         F77_FUNC(td04ad, TD04AD)(&rowcol_upper, &m, &p, index,
                                  dcoeff_cm, &lddcoe, ucoeff_cm, &lduco1, &lduco2,
                                  nr, a, &lda, b, &ldb, c, &ldc, d, &ldd,
                                  &tol, iwork, dwork, &ldwork, &info,
                                  rowcol_len);
         // NR, A, B, C, D, IWORK modified in place.
     }
