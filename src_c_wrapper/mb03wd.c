                  ...
                  int row_major) // row_major ignored for 'h', 'z'
{
    ...
    if (n == 0) { 
        ldwork_val = MAX(1, p - 2); // IHI-ILO = -1 for N=0
    } else { 
    ...
    /* --- Prepare pointers for Fortran call --- */
    double* h_for_fortran = (n > 0) ? h : NULL; 
    double* z_for_fortran = (compz_upper != 'N' && n > 0) ? z : NULL;
    if (compz_upper == 'N') { // Explicitly NULL if Z not used, regardless of N
        z_for_fortran = NULL;
    }
