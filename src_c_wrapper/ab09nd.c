 ...
 */
                   ...
                   int* iwarn, int row_major)
 {
     ...
     if (row_major) {
         ...
         if (ldd < min_ldd_rm_cols) { info = -17; goto cleanup; }
     } else {
     ...
     F77_FUNC(ab09nd, AB09ND)(&dico_upper, &job_upper, &equil_upper, &ordsel_upper,
                              &n, &m, &p, nr, &alpha, a, &lda, b, &ldb, c, &ldc, d, &ldd,
                              ns, hsv, &tol1, &tol2, iwork, &dwork_query, &ldwork,
                              iwarn, &info,
     ...
     if (row_major) {
         ...
         F77_FUNC(ab09nd, AB09ND)(&dico_upper, &job_upper, &equil_upper, &ordsel_upper,
                                  ...
                                  iwarn, &info,
                                  dico_len, job_len, equil_len, ordsel_len);
         // A, B, C, D, NS, HSV are modified in place by the Fortran call.
     }
