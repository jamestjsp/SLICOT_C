                  ...
                  double actol, int row_major)
{
    ...
    // Calculate required DWORK size based on formula
    // M1 = M - M2, NP1 = NP - NP2, ND1 = NP1 - M2, ND2 = M1 - NP2
    int nd1 = np1 - m2;     // NP1 - M2
    int nd2 = m1 - np2;     // M1 - NP2
    
    // Calculate LW1 through LW7 components
    int lw1 = n*m + np*n + np*m + m2*m2 + np2*np2;
    
    // LW2 calculation with multiple max operations
    int lw2_part1 = (n + np1 + 1) * (n + m2) + 
                     MAX(3*(n + m2) + n + np1, 5*(n + m2));
    
    int lw2_part2 = (n + np2) * (n + m1 + 1) + 
                     MAX(3*(n + np2) + n + m1, 5*(n + np2));
    
    int lw2_part3 = m2 + np1*np1 + MAX(MAX(np1*MAX(n, m1), 3*m2 + np1), 5*m2);
    
    int lw2_part4 = np2 + m1*m1 + MAX(MAX(MAX(n, np1)*m1, 3*np2 + m1), 5*np2);
    
    int lw2 = MAX(MAX(lw2_part1, lw2_part2), MAX(lw2_part3, lw2_part4));
    
    // LW3 calculation
    int lw3_part1 = nd1*m1 + MAX(4*MIN(nd1, m1) + MAX(nd1, m1), 6*MIN(nd1, m1));
    int lw3_part2 = np1*nd2 + MAX(4*MIN(np1, nd2) + MAX(np1, nd2), 6*MIN(np1, nd2));
    int lw3 = MAX(lw3_part1, lw3_part2);
    ...
    // LW6 calculation
    int lw6_part1 = m*m + MAX(2*m1, 3*n*n + MAX(n*m, 10*n*n + 12*n + 5));
    int lw6_part2 = np*np + MAX(2*np1, 3*n*n + MAX(n*np, 10*n*n + 12*n + 5));
    int lw6 = MAX(lw6_part1, lw6_part2);
    
    // LW7 calculation
    int lw7_part1 = nd1*nd1 + MAX(2*nd1, (nd1 + nd2)*np2);
    int lw7_part2 = nd2*nd2 + MAX(2*nd2, nd2*m2);
    int lw7_part3 = n*(2*np2 + m2) + MAX(2*n*m2, m2*np2 + MAX(m2*m2 + 3*m2, 
                           np2*(2*np2 + m2 + MAX(np2, n))));
    int lw7 = m2*np2 + np2*np2 + m2*m2 + MAX(MAX(lw7_part1, lw7_part2), MAX(3*n, lw7_part3));
    
    // Final LDWORK calculation
    ldwork = lw1 + MAX(1, MAX(MAX(MAX(lw2, lw3), lw4), lw5 + MAX(lw6, lw7)));
