 ...
 */
                   ...
                   int* ranke, int* rnka22, double tol, int row_major)
 {
     ...
     if (tol >= 1.0) { info = -21; goto cleanup; } // TOL must be < 1
     ...
     int min_ldq_f = (compq_upper == 'N') ? 1 : MAX(1, l);
     int min_ldz_f = (compz_upper == 'N') ? 1 : MAX(1, n);
     ...
     if (row_major) {
         ...
         int min_ldq_rm_cols = (compq_upper == 'N') ? 1 : l; // Q is L x L
         int min_ldz_rm_cols = (compz_upper == 'N') ? 1 : n; // Z is N x N
         if (lda < min_lda_rm_cols) { info = -9; goto cleanup; }
         if (lde < min_lde_rm_cols) { info = -11; goto cleanup; }
         if (m > 0 && ldb < min_ldb_rm_cols) { info = -13; goto cleanup; }
         ...
         if (ldb < min_ldb_f) { info = -13; goto cleanup; } // Check even if m=0? Fortran requires LDB>=1
     ...
     F77_FUNC(tg01fd, TG01FD)(&compq_upper, &compz_upper, &joba_upper, &l, &n, &m, &p,
                              NULL, &lda, NULL, &lde, NULL, &ldb, NULL, &ldc,
                              NULL, &ldq, NULL, &ldz,
     ...
     if (row_major) {
         ...
         if (compq_upper == 'U' && q_size > 0) { // Copy Q if updating
             slicot_transpose_to_fortran(q, q_cm, q_rows, q_cols, elem_size);
         } else if (compq_upper == 'I' && q_size > 0) { // Initialize Q if requested
             set_identity(l, q_cm, l, 0); // Initialize CM Q to identity
         }
         if (compz_upper == 'U' && z_size > 0) { // Copy Z if updating
             slicot_transpose_to_fortran(z, z_cm, z_rows, z_cols, elem_size);
         } else if (compz_upper == 'I' && z_size > 0) { // Initialize Z if requested
         ...
         if (info == 0) {
             ...
             if (compq_upper != 'N' && q_size > 0) slicot_transpose_to_c(q_cm, q, q_rows, q_cols, elem_size);
             if (compz_upper != 'N' && z_size > 0) slicot_transpose_to_c(z_cm, z, z_rows, z_cols, elem_size);
         ...
         if (compq_upper == 'I') set_identity(l, q, ldq, 0);
         if (compz_upper == 'I') set_identity(n, z, ldz, 0);
         ...
         F77_FUNC(tg01fd, TG01FD)(&compq_upper, &compz_upper, &joba_upper, &l, &n, &m, &p,
                                  ...
                                  iwork, dwork, &ldwork, &info,
                                  compq_len, compz_len, joba_len);
         // A, E, B, C, Q, Z, RANKE, RNKA22 modified in place.
     }
