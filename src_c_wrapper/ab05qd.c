/**
 * @file ab05qd.c
 * @brief C wrapper implementation for SLICOT routine AB05QD
 *
 * This file provides a C wrapper implementation for the SLICOT routine AB05QD,
 ...
 */
  ...
  */
 extern void F77_FUNC(ab05qd, AB05QD)(
 ...
 int slicot_ab05qd(char over,
                   ...
                   int* n_out, int* m_out, int* p_out, // Renamed to avoid conflict
                   ...
                   int row_major)
 {
     ...
     int m = 0; // Calculated total input dimension
     int p = 0; // Calculated total output dimension
     ...
     // Calculate resulting dimensions N, M, P
     n = n1 + n2;
     m = m1 + m2;
     p = p1 + p2;
     if (n_out) { *n_out = n; } // Set output N
     if (m_out) { *m_out = m; } // Set output M
     if (p_out) { *p_out = p; } // Set output P
     ...
     int min_ldc2_f = MAX(1, p2); int min_ldd2_f = MAX(1, p2); // Rows=P2
     // Check leading dimensions for outputs
     int min_lda_f = MAX(1, n); int min_ldb_f = MAX(1, n);
     int min_ldc_f = MAX(1, p); int min_ldd_f = MAX(1, p); // Rows=P

     if (row_major) {
         // Check C dimensions (number of columns)
         if (n1 > 0 && lda1 < n1) { info = -9; goto cleanup; }
         if (n1 > 0 && ldb1 < m1) { info = -11; goto cleanup; } // B1 is N1-by-M1
         if (p1 > 0 && ldc1 < n1) { info = -13; goto cleanup; } // C1 is P1-by-N1
         if (p1 > 0 && ldd1 < m1) { info = -15; goto cleanup; } // D1 is P1-by-M1
         if (n2 > 0 && lda2 < n2) { info = -17; goto cleanup; }
         if (n2 > 0 && ldb2 < m2) { info = -19; goto cleanup; } // B2 is N2-by-M2
         if (p2 > 0 && ldc2 < n2) { info = -21; goto cleanup; } // C2 is P2-by-N2
         if (p2 > 0 && ldd2 < m2) { info = -23; goto cleanup; } // D2 is P2-by-M2
         // Check output C dimensions
         if (n > 0 && lda < n) { info = -28; goto cleanup; }
         if (n > 0 && ldb < m) { info = -30; goto cleanup; } // B is N-by-M
         if (p > 0 && ldc < n) { info = -32; goto cleanup; } // C is P-by-N
         if (p > 0 && ldd < m) { info = -34; goto cleanup; } // D is P-by-M
     } else {
         ...
         if (lda < min_lda_f) { info = -28; goto cleanup; }
         if (ldb < min_ldb_f) { info = -30; goto cleanup; }
         if (ldc < min_ldc_f) { info = -32; goto cleanup; }
         if (ldd < min_ldd_f) { info = -34; goto cleanup; }
     }

     /* --- Workspace Allocation (None needed for AB05QD) --- */
     ...
     if (row_major) {
         ...
         size_t b1_size = (size_t)n1 * m1; if (n1 == 0 || m1 == 0) b1_size = 0; // N1 rows, M1 cols
         ...
         size_t b2_size = (size_t)n2 * m2; if (n2 == 0 || m2 == 0) b2_size = 0; // N2 rows, M2 cols
         size_t c2_size = (size_t)p2 * n2; if (p2 == 0 || n2 == 0) c2_size = 0; // P2 rows, N2 cols
         size_t d2_size = (size_t)p2 * m2; if (p2 == 0 || m2 == 0) d2_size = 0; // P2 rows, M2 cols
         ...
         size_t c_size = (size_t)p * n; if (p == 0 || n == 0) c_size = 0; // P rows, N cols
         size_t d_size = (size_t)p * m; if (p == 0 || m == 0) d_size = 0; // P rows, M cols
         ...
         if (b1_size > 0) slicot_transpose_to_fortran(b1, b1_cm, n1, m1, sizeof(double)); // N1 rows, M1 cols
         ...
         if (b2_size > 0) slicot_transpose_to_fortran(b2, b2_cm, n2, m2, sizeof(double)); // N2 rows, M2 cols
         if (c2_size > 0) slicot_transpose_to_fortran(c2, c2_cm, p2, n2, sizeof(double)); // P2 rows, N2 cols
         if (d2_size > 0) slicot_transpose_to_fortran(d2, d2_cm, p2, m2, sizeof(double)); // P2 rows, M2 cols
         ...
         ldc2_f = MAX(1, p2); ldd2_f = MAX(1, p2);
         lda_f  = MAX(1, n);  ldb_f  = MAX(1, n);
         ldc_f  = MAX(1, p);  ldd_f  = MAX(1, p);
     ...
     if (row_major && info == 0) {
         ...
         if (c_size > 0) slicot_transpose_to_c(c_cm, c, p, n, sizeof(double)); // P rows, N cols
         if (d_size > 0) slicot_transpose_to_c(d_cm, d, p, m, sizeof(double)); // P rows, M cols
     }
