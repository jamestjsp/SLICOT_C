 ...
 */
                   ...
                   int row_major)
 {
     ...
     int p_calc = 0;       // Calculated total output dimension
     ...
     int min_ldc2_f = (n2 > 0) ? MAX(1, p2) : 1; int min_ldd2_f = MAX(1, p2);
     // Check leading dimensions for outputs
     int min_lda_f = MAX(1, n_calc); int min_ldb_f = MAX(1, n_calc);
     int min_ldc_f = (n_calc > 0) ? MAX(1, p_calc) : 1; int min_ldd_f = MAX(1, p_calc); // Rows=P
     ...
     if (row_major) {
         ...
         if (lda < n_calc) { info = -28; goto cleanup; }
         if (ldb < m_calc) { info = -30; goto cleanup; } // Cols=M
         if (ldc < n_calc) { info = -32; goto cleanup; }
         if (ldd < m_calc) { info = -34; goto cleanup; } // Cols=M
     } else {
     ...
     if (row_major) {
         ...
         size_t c2_size = (size_t)p2 * n2; if (p2 == 0 || n2 == 0) c2_size = 0; // Rows=P2
         size_t d2_size = (size_t)p2 * m2; if (p2 == 0 || m2 == 0) d2_size = 0; // Rows=P2
         ...
         size_t c_size = (size_t)p_calc * n_calc; if (p_calc == 0 || n_calc == 0) c_size = 0; // Rows=P
         size_t d_size = (size_t)p_calc * m_calc; if (p_calc == 0 || m_calc == 0) d_size = 0; // Rows=P
         ...
         if (c2_size > 0) slicot_transpose_to_fortran(c2, c2_cm, p2, n2, sizeof(double)); // Rows=P2
         if (d2_size > 0) slicot_transpose_to_fortran(d2, d2_cm, p2, m2, sizeof(double)); // Rows=P2
         ...
         int ldc2_f = (p2 > 0) ? p2 : 1; int ldd2_f = (p2 > 0) ? p2 : 1; // Rows=P2
         int lda_f = (n_calc > 0) ? n_calc : 1; int ldb_f = (n_calc > 0) ? n_calc : 1;
         int ldc_f = (p_calc > 0) ? p_calc : 1; int ldd_f = (p_calc > 0) ? p_calc : 1; // Rows=P
         ...
         F77_FUNC(ab05qd, AB05QD)(&over_upper,
                                  ...
                                  &n_calc, &m_calc, &p_calc, // Pass calculated N, M, P
         ...
         if (info == 0) {
             ...
             if (c_size > 0) slicot_transpose_to_c(c_cm, c, p_calc, n_calc, sizeof(double)); // Rows=P
             if (d_size > 0) slicot_transpose_to_c(d_cm, d, p_calc, m_calc, sizeof(double)); // Rows=P
         }
