 ...
 */
                   ...
                   int row_major)
 {
     /* Local variables */
     int info = 0;
     int ldwork = 1;       // Not used if OVER='N'
     double* dwork = NULL; // Not used if OVER='N'
     ...
     if (uplo_upper != 'U' && uplo_upper != 'L') { info = -1; goto cleanup; }
     ...
     if (over_upper != 'N') { info = -2; goto cleanup; } // Should not happen now
     ...
     int min_ldc_f = (n_calc > 0) ? MAX(1, p2) : 1; int min_ldd_f = MAX(1, p2);
     ...
     /* --- Workspace Allocation (Not needed for OVER = 'N') --- */
     // ldwork = 1;
     // dwork = NULL; // Pass NULL for dwork
     ...
     if (row_major) {
         ...
         size_t b2_size = (size_t)n2 * p1; if (n2 == 0 || p1 == 0) b2_size = 0;
         size_t c2_size = (size_t)p2 * n2; if (p2 == 0 || n2 == 0) c2_size = 0;
         size_t d2_size = (size_t)p2 * p1; if (p2 == 0 || p1 == 0) d2_size = 0;
         ...
         size_t b_size = (size_t)n_calc * m1; if (n_calc == 0 || m1 == 0) b_size = 0;
         size_t c_size = (size_t)p2 * n_calc; if (p2 == 0 || n_calc == 0) c_size = 0;
         size_t d_size = (size_t)p2 * m1; if (p2 == 0 || m1 == 0) d_size = 0;
         ...
         int ldc1_f = (p1 > 0) ? p1 : 1; int ldd1_f = (p1 > 0) ? p1 : 1; // Adjusted LDC1 based on rows=P1
         int lda2_f = (n2 > 0) ? n2 : 1; int ldb2_f = (n2 > 0) ? n2 : 1;
         int ldc2_f = (p2 > 0) ? p2 : 1; int ldd2_f = (p2 > 0) ? p2 : 1; // Adjusted LDC2 based on rows=P2
         int lda_f = (n_calc > 0) ? n_calc : 1; int ldb_f = (n_calc > 0) ? n_calc : 1;
         int ldc_f = (p2 > 0) ? p2 : 1; int ldd_f = (p2 > 0) ? p2 : 1; // Adjusted LDC based on rows=P2
         ...
         F77_FUNC(ab05md, AB05MD)(&uplo_upper, &over_upper,
                                  ...
                                  &n_calc, // Pass calculated N, Fortran will verify
                                  a_cm, &lda_f, b_cm, &ldb_f,
                                  c_cm, &ldc_f, d_cm, &ldd_f,
                                  dwork, &ldwork, &info, // Pass NULL, 1 for dwork
                                  uplo_len, over_len);
 ...
 cleanup:
     ...
     free(dwork); // Should be NULL if OVER='N'
