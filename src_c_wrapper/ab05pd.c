 ...
 */
 ...
 extern void F77_FUNC(ab05pd, AB05PD)(
     ...
     const int* m,           // INTEGER M (input M)
     const int* p,           // INTEGER P (input P)
 ...
 int slicot_ab05pd(char over,
                   int n1, int m_in, int p_in, int n2, double alpha, // Use m_in, p_in for clarity
                   ...
                   int* n_out, // Renamed to avoid conflict
                   ...
                   int row_major)
 {
     ...
     int n = 0; // Calculated total state dimension
     ...
     n = n1 + n2;
     if (n_out) { *n_out = n; } // Set output N

     // Check leading dimensions for inputs
     int min_lda1_f = MAX(1, n1); int min_ldb1_f = MAX(1, n1);
     int min_ldc1_f = MAX(1, p_in); int min_ldd1_f = MAX(1, p_in); // Rows=P
     int min_lda2_f = MAX(1, n2); int min_ldb2_f = MAX(1, n2);
     int min_ldc2_f = MAX(1, p_in); int min_ldd2_f = MAX(1, p_in); // Rows=P
     // Check leading dimensions for outputs
     int min_lda_f = MAX(1, n); int min_ldb_f = MAX(1, n);
     int min_ldc_f = MAX(1, p_in); int min_ldd_f = MAX(1, p_in); // Rows=P

     if (row_major) {
         // Check C dimensions (number of columns)
         if (n1 > 0 && lda1 < n1) { info = -8; goto cleanup; }
         if (n1 > 0 && ldb1 < m_in) { info = -10; goto cleanup; } // B1 is N1-by-M
         if (p_in > 0 && ldc1 < n1) { info = -12; goto cleanup; } // C1 is P-by-N1
         if (p_in > 0 && ldd1 < m_in) { info = -14; goto cleanup; } // D1 is P-by-M
         if (n2 > 0 && lda2 < n2) { info = -16; goto cleanup; }
         if (n2 > 0 && ldb2 < m_in) { info = -18; goto cleanup; } // B2 is N2-by-M
         if (p_in > 0 && ldc2 < n2) { info = -20; goto cleanup; } // C2 is P-by-N2
         if (p_in > 0 && ldd2 < m_in) { info = -22; goto cleanup; } // D2 is P-by-M
         // Check output C dimensions
         if (n > 0 && lda < n) { info = -25; goto cleanup; }
         if (n > 0 && ldb < m_in) { info = -27; goto cleanup; } // B is N-by-M
         if (p_in > 0 && ldc < n) { info = -29; goto cleanup; } // C is P-by-N
         if (p_in > 0 && ldd < m_in) { info = -31; goto cleanup; } // D is P-by-M
     } else {
     ...
     if (row_major) {
         ...
         size_t b1_size = (size_t)n1 * m_in; if (n1 == 0 || m_in == 0) b1_size = 0; // N1 rows, M cols
         size_t c1_size = (size_t)p_in * n1; if (p_in == 0 || n1 == 0) c1_size = 0; // P rows, N1 cols
         size_t d1_size = (size_t)p_in * m_in; if (p_in == 0 || m_in == 0) d1_size = 0; // P rows, M cols
         size_t a2_size = (size_t)n2 * n2; if (n2 == 0) a2_size = 0;
         size_t b2_size = (size_t)n2 * m_in; if (n2 == 0 || m_in == 0) b2_size = 0; // N2 rows, M cols
         size_t c2_size = (size_t)p_in * n2; if (p_in == 0 || n2 == 0) c2_size = 0; // P rows, N2 cols
         size_t d2_size = (size_t)p_in * m_in; if (p_in == 0 || m_in == 0) d2_size = 0; // P rows, M cols
         ...
         size_t b_size = (size_t)n * m_in; if (n == 0 || m_in == 0) b_size = 0; // N rows, M cols
         size_t c_size = (size_t)p_in * n; if (p_in == 0 || n == 0) c_size = 0; // P rows, N cols
         size_t d_size = (size_t)p_in * m_in; if (p_in == 0 || m_in == 0) d_size = 0; // P rows, M cols
         ...
         if (b1_size > 0) slicot_transpose_to_fortran(b1, b1_cm, n1, m_in, sizeof(double)); // N1 rows, M cols
         if (c1_size > 0) slicot_transpose_to_fortran(c1, c1_cm, p_in, n1, sizeof(double)); // P rows, N1 cols
         if (d1_size > 0) slicot_transpose_to_fortran(d1, d1_cm, p_in, m_in, sizeof(double)); // P rows, M cols
         if (a2_size > 0) slicot_transpose_to_fortran(a2, a2_cm, n2, n2, sizeof(double));
         if (b2_size > 0) slicot_transpose_to_fortran(b2, b2_cm, n2, m_in, sizeof(double)); // N2 rows, M cols
         if (c2_size > 0) slicot_transpose_to_fortran(c2, c2_cm, p_in, n2, sizeof(double)); // P rows, N2 cols
         if (d2_size > 0) slicot_transpose_to_fortran(d2, d2_cm, p_in, m_in, sizeof(double)); // P rows, M cols

         /* Fortran leading dimensions (number of rows) */
         lda1_f = MAX(1, n1); ldb1_f = MAX(1, n1);
         ldc1_f = MAX(1, p_in); ldd1_f = MAX(1, p_in);
         lda2_f = MAX(1, n2); ldb2_f = MAX(1, n2);
         ldc2_f = MAX(1, p_in); ldd2_f = MAX(1, p_in);
         lda_f  = MAX(1, n);  ldb_f  = MAX(1, n);
         ldc_f  = MAX(1, p_in); ldd_f  = MAX(1, p_in);
     ...
     if (row_major && info == 0) {
         ...
         if (b_size > 0) slicot_transpose_to_c(b_cm, b, n, m_in, sizeof(double)); // N rows, M cols
         if (c_size > 0) slicot_transpose_to_c(c_cm, c, p_in, n, sizeof(double)); // P rows, N cols
         if (d_size > 0) slicot_transpose_to_c(d_cm, d, p_in, m_in, sizeof(double)); // P rows, M cols
     }
