/**
 * @file sb10hd.c
 * @brief C wrapper implementation for SLICOT routine SB10HD
 *
 * This file provides a C wrapper implementation for the SLICOT routine SB10HD,
 ...
 */
                   ...
                   double* rcond, double tol, int row_major)
 {
     ...
     if (row_major) {
         ...
         if (ldak < min_ldak_rm_cols) { info = -15; goto cleanup; }
         if (ldbk < min_ldbk_rm_cols) { info = -17; goto cleanup; }
         if (ldck < min_ldck_rm_cols) { info = -19; goto cleanup; }
         if (lddk < min_lddk_rm_cols) { info = -21; goto cleanup; }
     } else {
         ...
         if (ldak < min_ldak_f) { info = -15; goto cleanup; }
         if (ldbk < min_ldbk_f) { info = -17; goto cleanup; }
         if (ldck < min_ldck_f) { info = -19; goto cleanup; }
         if (lddk < min_lddk_f) { info = -21; goto cleanup; }
     }
 
     /* --- Workspace Allocation --- */
 
     // Determine fixed workspace sizes
     liwork = MAX(1, MAX(2 * n, n * n));
     lbwork = MAX(1, 2 * n);
     ...
     if (info < 0 && info != -26) { info = info; goto cleanup; } // Query failed due to invalid argument
